...
export function trivyDashboard(): Record<string, unknown> {
  return {
    ...
    panels: [
      ...
      {
        id: 6,
        title: "Vulnerabilities by Exposure",
        type: "bargauge",
        gridPos: { h: 8, w: 12, x: 0, y: 26 },
        datasource: PROM_DS,
        targets: [
          {
            expr: "sum by (exposure, severity) (nimbus_trivy_vulnerability_findings)",
            refId: "A",
            legendFormat: "{{exposure}} / {{severity}}",
          },
        ],
        options: { orientation: "horizontal", displayMode: "gradient" },
        fieldConfig: { defaults: { min: 0 }, overrides: [] },
      ...
      {
        id: 7,
        title: "Externally Reachable Critical and High CVEs",
        type: "table",
        gridPos: { h: 8, w: 12, x: 12, y: 26 },
        datasource: PROM_DS,
        targets: [
          {
            expr: 'topk(20, sum by (namespace, resource_kind, resource, image, severity) (nimbus_trivy_vulnerability_findings{exposure="external",severity=~"critical|high"}))',
            refId: "A",
            format: "table",
            instant: true,
          },
