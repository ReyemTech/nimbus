...
function deployUptimeKuma(
  ...
  const reconcilerScript = [
    ...
    '  const loginRes = await new Promise((resolve) => socket.emit("login", { username: USERNAME, password: PASSWORD, token: "" }, resolve));',
    '  if (!loginRes.ok) { console.error("Login failed:", loginRes.msg); process.exit(1); }',
